...
unsigned long largestRegion = 0;
unsigned long curRegion = 1;
float averageRegion = 0;
unsigned long iteration = 0;
unsigned long previousPFN = 0; 
unsigned long continuousVP = 0;
unsigned long numVP = 0;
...
static void print_page(unsigned long address, unsigned long data,
    ...
    if(pfn != 0) {
            ...
            if((pfn == (previousPFN+1)) || (pfn == (previousPFN-1))) {
                continuousVP++;
                curRegion++;
            }
            else  {
            ...
            printf("continuousVP: %lu\t totalVP: %lu\t continuous ratio: %f\ncurrent region: %lu\t average region: %f\tlargest region: %lu\n", continuousVP, numVP, contRatio, curRegion, averageRegion, largestRegion);
    }
}
